...
def rbf_kernel(xx, p_var = 1.0):
    # N x 1
    XY = tf.matmul(xx, tf.transpose(xx))
    X2 = tf.reduce_sum(tf.square(xx), axis=1, keepdims=True)
    H = X2 + tf.transpose(X2) - 2 * XY

    # V = tf.reshape(H, [-1])
    median = tf.contrib.distributions.percentile(H, q=50.)
    
    Kxy = tf.exp( - H / (median * (p_var ** 2)))
