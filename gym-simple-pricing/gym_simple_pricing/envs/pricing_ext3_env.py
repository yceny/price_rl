...
class PricingExt3Env(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low = self.priceLow, high = self.priceHigh, shape=(3,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        PSprobBuySP = math.exp(-(self.PSpriceSensitivity*price)**self.PSpriceScale)*(1-math.exp(-(self.PSqualitySensitivity*quality_ps)**self.PSqualityScale))
        ...
        QSprobBuySP = math.exp(-(self.QSpriceSensitivity*price)**self.QSpriceScale)*(1-math.exp(-(self.QSqualitySensitivity*quality_qs)**self.QSqualityScale))
