...
## NOTE: class name has been changed ##

class PricingExt1Env(gym.Env):
    ...
    #
    # Extension 1: consider the option of food donation
    #  
    # Assumptions: 
    ...
    def __init__(self):
        ...
        self.qualityLow = 0.
        self.qualityHigh = 1.
        self.taxRate = 0.35 # assume tax rate of donation is 35%
    ...
    def step(self, action):
        ...
        if self._cur_episode_step >= self._max_episode_steps:
            ...
            ## add salvage value from donation, assume parameters a = 1, and b = 0.5

            if 1 * self.unitOrderingCost > 0.5 * (action[0] - self.unitOrderingCost):
                reward = reward - (inventoryLevel * self.unitOrderingCost) + 0.5 * (action[0] - self.unitOrderingCost) * inventoryLevel * self.taxRate
            else:
                reward = reward - (inventoryLevel * self.unitOrderingCost) + 1 * self.unitOrderingCost * inventoryLevel * self.taxRate
